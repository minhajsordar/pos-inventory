<template>
  <q-page padding>
    <div class="w-full flex justify-end pb-3">
      <template v-if="userAuthStore.checkShowOnPermission('createPurchase')">
        <q-btn
          style="background: green; color: white"
          label="New Purchase"
          to="/dashboard/purchase/purchase_list/create"
        />
      </template>
    </div>
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
      <CompactQTable
        :columns="columns"
        :data-list="purchaseList?.purchases || []"
        :page="purchaseList?.page || 1"
        :pages="purchaseList?.pages || 1"
        @updatePaginate="updatePaginate"
        @onEdit="onEdit"
        @onDelete="onDelete"
        @onFilter="onFilter"
        :showEdit="userAuthStore.checkShowOnPermission('editPurchase')"
        :showDelete="userAuthStore.checkShowOnPermission('deletePurchase')"
      />
...
const onEdit = (e) => {
  router.push(`/dashboard/purchase/purchase_list/${e._id}`);
};
const onDelete = (e) => {
...
const getPurchaseList = async () => {
  ...
  const config = {
    method: 'GET',
    url: 'api/purchase/purchase_lists',
    headers: {
    ...
    params: {
      ...
      select: ' supplier amount paidAmount purchaseId',
    },
  };
  ...
  try {
    ...
    response.data.purchases = response.data.purchases.map(e=>{
      if(e.supplier?.companyName){
        e.supplier = e.supplier?.companyName
      }
      return e;
    })
...
watch(paginationAndFilter, debounce(getPurchaseList, 1000));
const deletePurchase = async () => {
  // userAuthStore.checkLogin()
  const config = {
    method: 'delete',
    url: 'api/purchase/purchase_lists/' + deleteId.value,
    headers: {
      Authorization: `Bearer ${getToken('token')}`,
    },
  };
  try {
    ...
    const purchases = purchaseList?.value.purchases.filter(
      (item) => item._id !== deleteId.value,
    );
    purchaseList.value = {
      ...JSON.parse(JSON.stringify(purchaseList?.value)),
      purchases,
    };
    if (
      purchaseList?.value.purchases.length < 5 &&
      purchaseList?.value.pages !== 1
    ) {
      getPurchaseList();
    }
    $q.loading.hide();
