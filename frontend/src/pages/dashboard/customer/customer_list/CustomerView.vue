<template>
  <q-page padding>
    <div class="w-full flex justify-end pb-3">
      <template v-if="userAuthStore.checkShowOnPermission('createCustomer')">
        <q-btn
          style="background: green; color: white"
          label="New Customer"
          to="/dashboard/customer/customer_list/create"
        />
      </template>
    </div>
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
      <CompactQTable
        :columns="columns"
        :data-list="customerList?.customers || []"
        :page="customerList?.page || 1"
        :pages="customerList?.pages || 1"
        @updatePaginate="updatePaginate"
        @onEdit="onEdit"
        @onDelete="onDelete"
        @onFilter="onFilter"
        :showEdit="userAuthStore.checkShowOnPermission('editCustomer')"
        :showDelete="userAuthStore.checkShowOnPermission('deleteCustomer')"
      />
...
const onEdit = (e) => {
  router.push(`/dashboard/customer/customer_list/${e._id}`);
};
const onDelete = (e) => {
...
watch(paginationAndFilter, debounce(getCustomerList, 1000));
const deleteCustomer = async () => {
  // userAuthStore.checkLogin()
  const config = {
    method: 'delete',
    url: 'api/customer/customer_lists/' + deleteId.value,
    headers: {
      Authorization: `Bearer ${getToken('token')}`,
    },
  };
  try {
    ...
    const customers = customerList?.value.customers.filter(
      (item) => item._id !== deleteId.value,
    );
    customerList.value = {
      ...JSON.parse(JSON.stringify(customerList?.value)),
      customers,
    };
    if (
      customerList?.value.customers.length < 5 &&
      customerList?.value.pages !== 1
    ) {
      getCustomerList();
    }
    $q.loading.hide();
