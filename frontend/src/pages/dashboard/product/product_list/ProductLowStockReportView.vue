<template>
  <q-page padding>
    <div class="relative overflow-x-auto">
      <LowStockReportQTable title="Low Stock Report" :columns="columns" :data-list="productList?.products || []"
        :page="productList?.page || 1" :pages="productList?.pages || 1" @updatePaginate="updatePaginate"
        @onEdit="onEdit" @onDelete="onDelete" @onFilter="onFilter"
        :showEdit="userAuthStore.checkShowOnPermission('editProduct')"
        :showDelete="userAuthStore.checkShowOnPermission('deleteProduct')" />
    ...
    <q-dialog v-model="deletePopup">
      <q-card style="width: 300px; max-width: 80vw">
        ...
        <q-card-actions align="right" class="bg-white text-teal">
          ...
          <q-btn flat label="Delete" v-close-popup @click="deleteProduct" color="red" />
        </q-card-actions>
      </q-card>
    </q-dialog>
  </q-page>
...
import LowStockReportQTable from '@/components/table/LowStockReportQTable.vue';
...
const onEdit = (e) => {
  router.push(`/dashboard/product/product_list/${e._id}`);
};
const onDelete = (e) => {
...
watch(paginationAndFilter, debounce(getProductList, 1000));
const deleteProduct = async () => {
  // userAuthStore.checkLogin()
  const config = {
    method: 'delete',
    url: 'api/product/product_lists/' + deleteId.value,
    headers: {
      Authorization: `Bearer ${getToken('token')}`,
    },
  };
  try {
    ...
    const products = productList?.value.products.filter(
      (item) => item._id !== deleteId.value,
    );
    productList.value = {
      ...JSON.parse(JSON.stringify(productList?.value)),
      products,
    };
    if (
      productList?.value.products.length < 5 &&
      productList?.value.pages !== 1
    ) {
      getProductList();
    }
    $q.loading.hide();
