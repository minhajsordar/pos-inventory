<template>
  <q-page padding>
    <div class="w-full flex justify-end pb-3">
      <template v-if="userAuthStore.checkShowOnPermission('createEmployee')">
        <q-btn
          style="background: green; color: white"
          label="New Employee"
          to="/dashboard/employee/employee_list/create"
        />
      </template>
    </div>
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
      <CompactQTable
        :columns="columns"
        :data-list="employeeList?.employees || []"
        :page="employeeList?.page || 1"
        :pages="employeeList?.pages || 1"
        @updatePaginate="updatePaginate"
        @onEdit="onEdit"
        @onDelete="onDelete"
        @onFilter="onFilter"
        :showEdit="userAuthStore.checkShowOnPermission('editEmployee')"
        :showDelete="userAuthStore.checkShowOnPermission('deleteEmployee')"
      />
...
const onEdit = (e) => {
  router.push(`/dashboard/employee/employee_list/${e._id}`);
};
const onDelete = (e) => {
...
const getEmployeeList = async () => {
  ...
  const config = {
    method: 'GET',
    url: 'api/employee/employee_lists',
    headers: {
    ...
    params: {
      ...
      select:
        ' name phone1 phone2 email address nidNo birthRegistrationNo fatherName motherName',
    },
  };
...
watch(paginationAndFilter, debounce(getEmployeeList, 1000));
const deleteEmployee = async () => {
  // userAuthStore.checkLogin()
  const config = {
    method: 'delete',
    url: 'api/employee/employee_lists/' + deleteId.value,
    headers: {
      Authorization: `Bearer ${getToken('token')}`,
    },
  };
  try {
    ...
    const employees = employeeList?.value.employees.filter(
    ...
    if (
      employeeList?.value.employees.length < 5 &&
      employeeList?.value.pages !== 1
    ) {
      getEmployeeList();
    }
    $q.loading.hide();
