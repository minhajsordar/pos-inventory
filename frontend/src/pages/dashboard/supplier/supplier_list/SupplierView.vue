<template>
  <q-page padding>
    <div class="w-full flex justify-end pb-3">
      <template v-if="userAuthStore.checkShowOnPermission('createSupplier')">
        <q-btn
          style="background: green; color: white"
          label="New Supplier"
          to="/dashboard/supplier/supplier_list/create"
        />
      </template>
    </div>
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
      <CompactQTable
        :columns="columns"
        :data-list="supplierList?.suppliers || []"
        :page="supplierList?.page || 1"
        :pages="supplierList?.pages || 1"
        @updatePaginate="updatePaginate"
        @onEdit="onEdit"
        @onDelete="onDelete"
        @onFilter="onFilter"
        :showEdit="userAuthStore.checkShowOnPermission('editSupplier')"
        :showDelete="userAuthStore.checkShowOnPermission('deleteSupplier')"
      />
...
const onEdit = (e) => {
  router.push(`/dashboard/supplier/supplier_list/${e._id}`);
};
const onDelete = (e) => {
...
const getSupplierList = async () => {
  ...
  const config = {
    ...
    params: {
      ...
      select: ' companyName details phone email address',
    },
  };
...
watch(paginationAndFilter, debounce(getSupplierList, 1000));
const deleteSupplier = async () => {
  // userAuthStore.checkLogin()
  const config = {
    method: 'delete',
    url: 'api/supplier/supplier_lists/' + deleteId.value,
    headers: {
      Authorization: `Bearer ${getToken('token')}`,
    },
  };
  try {
    ...
    const suppliers = supplierList?.value.suppliers.filter(
      (item) => item._id !== deleteId.value,
    );
    supplierList.value = {
      ...JSON.parse(JSON.stringify(supplierList?.value)),
      suppliers,
    };
    if (
      supplierList?.value.suppliers.length < 5 &&
      supplierList?.value.pages !== 1
    ) {
      getSupplierList();
    }
    $q.loading.hide();
