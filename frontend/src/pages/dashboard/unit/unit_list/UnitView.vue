<template>
  <q-page padding>
    <div class="w-full flex justify-end pb-3">
      <template v-if="userAuthStore.checkShowOnPermission('createUnit')">
        <q-btn
          style="background: green; color: white"
          label="New Unit"
          to="/dashboard/unit/unite_list/create"
        />
      </template>
    </div>
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
      <CompactQTable
        :columns="columns"
        :data-list="unitList?.units || []"
        :page="unitList?.page || 1"
        :pages="unitList?.pages || 1"
        @updatePaginate="updatePaginate"
        @onEdit="onEdit"
        @onDelete="onDelete"
        @onFilter="onFilter"
        :showEdit="userAuthStore.checkShowOnPermission('editUnit')"
        :showDelete="userAuthStore.checkShowOnPermission('deleteUnit')"
      />
...
const onEdit = (e) => {
  router.push(`/dashboard/unit/unite_list/${e._id}`);
};
const onDelete = (e) => {
...
const deleteUnit = async () => {
  ...
  const config = {
    method: 'delete',
    url: 'api/unit/unit_lists/' + deleteId.value,
    headers: {
      Authorization: `Bearer ${getToken('token')}`,
    },
  };
  try {
    ...
    const units = unitList?.value.units.filter(
      (item) => item._id !== deleteId.value,
    );
    unitList.value = { ...JSON.parse(JSON.stringify(unitList?.value)), units };
    if (unitList?.value.units.length < 5 && unitList?.value.pages !== 1) {
      getUnitList();
    }
    $q.loading.hide();
