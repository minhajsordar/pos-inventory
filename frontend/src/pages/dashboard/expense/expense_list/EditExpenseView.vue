<template>
  <q-page padding>
    ...
    <div class="flex flex-wrap -m-1">
      <div class="w-3/5 p-1">
        <q-card flat bordered class="my-card mb-2" :class="[$q.dark.isActive ? 'bg-grey-9' : 'bg-grey-2',expenseAmount == 0 ? 'opacity-60 pointer-events-none' : '']">
        ...
        <q-card flat bordered class="my-card" :class="$q.dark.isActive ? 'bg-grey-9' : 'bg-grey-2'">
          <q-card-section class="q-pb-none text-h6">
            <div class="flex justify-between"><div>Expense Items</div> <q-btn @click="showAddExpenseForm" label="Add Item" color="orange" /></div>
        ...
        <q-card v-if="showAddNewExpense" flat bordered class="my-card mb-2"
          ...
          <q-card-section>
            <div>
              ...
              <div class="mb-6">
                <q-input label="Item name " placeholder="Expense item name" v-model="newExpense"
                   outlined dense />
...
const getExpense = async () => {
  ...
  const config = {
    method: 'GET',
    url: 'api/expense/expense_lists/' + route.params.id,
    headers: {
  ...
  try {
    ...
    $q.loading.hide();
    expenseSupplier.value = response.data.supplier;
    cartList.value = response.data.itemList;
    ...
    expensePaidAmount.value = Number(response.data.amount) - Number(response.data.paidAmount);
...
const getExpenseList = async () => {
  ...
  try {
    $q.loading.show();
    const response = await api.request(config);
    expenseList.value = response.data.expenses.map((e) => {
      e.label = e.name;
      e.value = e._id;
      return e;
    });
...
const updateExpense = async (e) => {
  ...
  try {
    ...
    // } else {
    //   router.push('/dashboard/expense/expense_lists');
    // }
    getExpense()
    $q.notify({
