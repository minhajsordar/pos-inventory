<template>
  <q-page padding>
    <div class="w-full flex justify-end pb-3">
      <template v-if="userAuthStore.checkShowOnPermission('createUser')">
        <q-btn
          style="background: green; color: white"
          label="New User"
          to="/dashboard/user/user_list/create"
        />
      </template>
    </div>
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
      <CompactQTable
        :columns="columns"
        :data-list="userList?.users || []"
        :page="userList?.page || 1"
        :pages="userList?.pages || 1"
        @updatePaginate="updatePaginate"
        @onEdit="onEdit"
        @onDelete="onDelete"
        @onFilter="onFilter"
        :showEdit="userAuthStore.checkShowOnPermission('editUser')"
        :showDelete="userAuthStore.checkShowOnPermission('deleteUser')"
      />
...
const onEdit = (e) => {
  router.push(`/dashboard/user/user_list/${e._id}`);
};
const onDelete = (e) => {
